...
Open **http://<pc-ip>:8080** from another machine on the network, or
**http://localhost:8080** on the deployed PC, and sign in with the seeded admin
...
- This root `docker-compose.yml` is edge-only. It does not start the separate
  central server; central sync and ETP publishing are disabled unless explicitly
  configured to point at an external central deployment.
- Only the frontend is published, on port `8080` by default. The backend and
