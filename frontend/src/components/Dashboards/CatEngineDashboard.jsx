...
// EDR side-strip configuration — all CAT engine catalog channels.
const EDR_CHANNELS = [
    'cat_engine.battery_voltage',
    'cat_engine.pedal_position',
    'cat_engine.coolant_level',
    'cat_engine.fuel_temp',
    'cat_engine.total_fuel',
    'cat_engine.total_hours',
    'cat_engine.run_hours',
    'cat_engine.source_cmd',
    'cat_engine.status',
    'cat_engine.coolant_temp',
    'cat_engine.load',
    'cat_engine.rpm',
    'cat_engine.fuel_pressure',
    'cat_engine.fuel_rate',
    'cat_engine.oil_pressure'
];
const EDR_STRIPS = [
    {
        title: 'Engine',
...
export default function CatEngineDashboard() {
    ...
    useEffect(() => {
        ...
        if (isLiveRigPayload(cached) && cached.cat_engine) setData(cached.cat_engine);
                        ...
                        <Paper sx={{ p: 1.75, bgcolor: surface, border: `1px solid ${border}`, display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                            ...
                            <Box sx={{ ml: 'auto', textAlign: 'right', pr: 1, display: 'flex', gap: 3 }}>
                                <Box>
                                    ...
                                    <Typography variant="h6" sx={{ color: 'text.primary', fontWeight: 'bold' }}>{Number(data.run_hours || 0).toFixed(2)} <span style={{ fontSize: '0.55em', color: '#64748b' }}>HRS</span></Typography>
                                </Box>
                                <Box>
                                    ...
                                    <Typography variant="h6" sx={{ color: 'primary.main', fontWeight: 'bold' }}>{Number(data.total_hours || 0).toFixed(2)} <span style={{ fontSize: '0.55em', color: '#64748b' }}>HRS</span></Typography>
                                </Box>
                            </Box>
                        </Paper>
                    </Grid>
